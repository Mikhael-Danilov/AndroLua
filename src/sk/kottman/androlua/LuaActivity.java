...
public class LuaActivity extends Activity implements ServiceConnection {
	...
	protected void onActivityResult(int request, int result, Intent data) {
		service.invokeMethod(modTable, "onActivityResult", request, result,
				data);
	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		Object res = service
				.invokeMethod(modTable, "onCreateOptionsMenu", menu);
		return res != null ? (Boolean) res : false;
	}

	@Override
	public boolean onOptionsItemSelected(MenuItem item) {
		Object res = service.invokeMethod(modTable, "onOptionsItemSelected",
				item);
		if (res == null) {
	...
	public boolean onContextItemSelected(MenuItem item) {
		Object res = service.invokeMethod(modTable, "onContextItemSelected",
				item);
		if (res != null) {
	...
	public void injectFunction(JavaFunction f, String name) {
		try {
			f.register(name);

		} catch (Exception e) {
			Log.d("lua", "Cannot register " + name + e.getMessage());
		}
	}
	...
	public void onServiceConnected(ComponentName name, IBinder iservice) {
		...
		JavaFunction test = new JavaFunction(Lua.L) {
			@Override
			public int execute() throws LuaException {
				String cmd = getParam(2).getString();
				return 0;
			}
		};
		...
		JavaFunction callFromJavaTest = new JavaFunction(Lua.L){
			@Override
			public int execute() throws LuaException {
				Lua.L.getGlobal("test");
				Lua.L.pushString("Hello from Java");
				Lua.L.call(1, 0);
				return 0;
			}
		};
		
		injectFunction(callFromJavaTest,"test2");
	}
